  ...
  if (compact) {
    return (
      <div className="bg-white dark:bg-slate-800 rounded-lg border border-gray-200 dark:border-slate-700 p-4">
        ...
        <p className="text-sm text-gray-600 dark:text-gray-300 mt-2">{breakdown.overallExplanation}</p>
      </div>
    );
  }
  ...
  return (
    <div className="bg-white dark:bg-slate-800 rounded-lg border border-gray-200 dark:border-slate-700 shadow-sm">
      ...
      <div className="p-6 border-b border-gray-200 dark:border-slate-700">
        <div className="flex items-center justify-between">
          <div>
            <h3 className="text-lg font-semibold text-gray-900 dark:text-white">Breakthrough Score</h3>
            <p className="text-sm text-gray-500 dark:text-gray-400">11-factor analysis</p>
          </div>
        ...
        <p className="text-sm text-gray-600 dark:text-gray-300 mt-3">{breakdown.overallExplanation}</p>
      </div>

      {/* Radar Chart Placeholder */}
      {showRadar && radarData && (
        <div className="p-6 border-b border-gray-200 dark:border-slate-700">
          <h4 className="text-sm font-medium text-gray-700 dark:text-gray-300 mb-4">Multi-Dimensional View</h4>
      ...
      {/* Factor Breakdown */}
      <div className="p-6 border-b border-gray-200 dark:border-slate-700">
        <h4 className="text-sm font-medium text-gray-700 dark:text-gray-300 mb-4">Factor Breakdown</h4>
        <div className="space-y-3">
          {breakdown.factors.map((factor) => (
            <div
              key={factor.id}
              className={`${onFactorClick ? 'cursor-pointer hover:bg-gray-50 dark:hover:bg-slate-700' : ''} rounded-lg p-2 -mx-2`}
            ...
            >
              ...
              <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">{factor.explanation}</p>
            </div>
          ))}
        </div>
      </div>
        ...
        <div className="grid grid-cols-2 gap-4">
          <div>
            ...
            {breakdown.strengths.length > 0 ? (
              ...
              <p className="text-xs text-gray-500 dark:text-gray-400">No major strengths identified</p>
            )}
          </div>
          <div>
            ...
            {breakdown.weaknesses.length > 0 ? (
              ...
              <p className="text-xs text-gray-500 dark:text-gray-400">No major weaknesses identified</p>
            )}
          </div>
        </div>
      </div>
      ...
      {showSuggestions && suggestions.length > 0 && (
        <div className="p-6">
          <h4 className="text-sm font-medium text-gray-700 dark:text-gray-300 mb-3">Top Improvements</h4>
          <div className="space-y-2">
            {suggestions.slice(0, 3).map((suggestion, i) => (
              <div key={i} className="flex items-start bg-blue-50 dark:bg-blue-900/20 rounded-lg p-3">
                <div className="flex-shrink-0 w-6 h-6 bg-blue-100 dark:bg-blue-800 rounded-full flex items-center justify-center text-blue-600 dark:text-blue-300 text-xs font-semibold mr-3">
                  {i + 1}
                </div>
      ...
      {/* Metadata */}
      <div className="px-6 py-3 bg-gray-50 dark:bg-slate-900 rounded-b-lg">
