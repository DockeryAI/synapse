 ...
 * - Real data from 16+ intelligence sources (Serper 8 endpoints, Apify 3 actors, Website Analyzer, YouTube, OutScraper, News, Weather, SEMrush)
 ...
 *
 * Created: 2025-11-13
 * Updated: 2025-11-14 (Expanded to 16+ sources)
 */
...
export function SynapsePage() {
  ...
  /**
   * Auto-detect location when URL changes
   */
  ...
  const runSynapseDiscovery = async () => {
    ...
    try {
      ...
      const syntheticBrand = {
        id: 'demo',
        name: new URL(testUrl).hostname.split('.')[0] || 'Demo Business',
        website: testUrl,
      ...
      // Step 2: Transform DeepContext into format expected by Synapse Generator
      const businessIntel = {
        business: {
          name: new URL(testUrl).hostname.split('.')[0] || 'Business',
          industry: selectedIndustry.displayName,
          location: {
        ...
        // Include real data points from DeepContext
        customerTriggers: deepContextResult.context.synthesis.keyInsights.map((insight, i) => ({
          trigger: insight,
          source: 'deepcontext',
          confidence: 0.8,
          index: i
        })),
        trends: deepContextResult.context.realTimeCultural.trending.topics.map((topic, i) => ({
          topic,
          relevance: 0.7,
          index: i
        })),
        // Add opportunity score
        opportunityScore: deepContextResult.context.synthesis.opportunityScore
      };

      // Step 3: Run Synapse Discovery to find breakthrough insights
      console.log('[Synapse] Step 2/4: Discovering breakthrough connections...');
      const synapseResult = await generateSynapses({
        business: businessIntel.business,
        intelligence: businessIntel as any
      });

      console.log('[Synapse] ✅ Synapses discovered:', synapseResult.synapses.length);
      ...
      if (synapseResult.synapses.length > 0) {
        ...
        console.log('[Synapse] ✅ Content generated:', contentResult.content.length, 'pieces');
        ...
        <div className="text-center space-y-4">
          ...
          >
            ...
            <p className="text-lg text-gray-600 dark:text-slate-400">
              Breakthrough Content Discovery with 16+ Intelligence Sources
            </p>
            <p className="text-sm text-gray-500 dark:text-slate-500">
              Serper 8x • Apify 3x • Website AI • YouTube • OutScraper • News • Weather • SEMrush • OpenAI
            </p>
          </motion.div>
        </div>
          ...
          <div className="space-y-4">
            ...
            {(detectedLocation || selectedLocations.length > 0 || testUrl.length >= 10) && (
              ...
              >
                ...
                {!detectedLocation && testUrl.length >= 10 && selectedLocations.length === 0 && (
