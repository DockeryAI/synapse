...
class EasyModeService {
   ...
   * Now supports selectedInsights from tabs (triggers, gaps, trends, proof)
   */
  async generateQuickPost(
    ...
    options?: {
      ...
      /** Selected insights from tabs to incorporate into content */
      selectedInsights?: SelectedInsight[];
    }
  ): Promise<QuickPostResult> {
    console.log('[Easy Mode] Generating quick post...');
    if (options?.selectedInsights?.length) {
      console.log(`[Easy Mode] Using ${options.selectedInsights.length} selected insights from tabs`);
    }
    ...
    try {
      ...
      // Generate primary post - pass selected insights to orchestrator
      const content = await contentOrchestrator.generate({
        uvp,
        pillar,
        platform: platform as any,
        ...
        selectedInsights: options?.selectedInsights // Wire through selected insights from tabs
      });
