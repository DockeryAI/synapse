...
import { synapseDataProvider } from './synapse-data-provider.service';
...
const DEFAULT_MODEL = 'anthropic/claude-opus-4';   // Best quality - Opus 4.5 (OpenRouter format)
...
class ContentOrchestrator {
  ...
  /**
   * Build brand voice guidance for the prompt
   */
  private buildBrandVoicePrompt(brandVoice: NonNullable<CompleteUVP['brandVoice']>): string {
    const sections: string[] = [
      '\n\n=== BRAND VOICE & TONE ===',
      'CRITICAL: Match this exact brand voice in all generated content.',
      ''
    ];

    if (brandVoice.tone.length > 0) {
      sections.push(`Primary Tone: ${brandVoice.tone.join(', ')}`);
    }

    if (brandVoice.values.length > 0) {
      sections.push(`Brand Values: ${brandVoice.values.join(', ')}`);
    }

    if (brandVoice.personality.length > 0) {
      sections.push(`Personality: ${brandVoice.personality.join(', ')}`);
    }

    if (brandVoice.vocabularyPatterns.length > 0) {
      sections.push(`Power Words to Use: ${brandVoice.vocabularyPatterns.slice(0, 8).join(', ')}`);
    }

    if (brandVoice.avoidWords.length > 0) {
      sections.push(`Words/Styles to AVOID: ${brandVoice.avoidWords.join(', ')}`);
    }

    if (brandVoice.signaturePhrases.length > 0) {
      sections.push(`Signature Phrases: "${brandVoice.signaturePhrases.slice(0, 3).join('", "')}"`);
    }

    sections.push(`Sentence Style: ${brandVoice.sentenceStyle}`);
    sections.push(`Emotional Temperature: ${brandVoice.emotionalTemperature}`);

    sections.push('');
    sections.push('Use their vocabulary patterns and match their communication style exactly.');
    sections.push('=== END BRAND VOICE ===\n');

    console.log(`[V4 Orchestrator] Adding brand voice guidance: ${brandVoice.tone.join(', ')}`);
    return sections.join('\n');
  }

  /**
   * Build customer stories prompt for grounding content in real data
   */
  private buildCustomerStoriesPrompt(stories: string[]): string {
    if (stories.length === 0) return '';

    const sections: string[] = [
      '\n\n=== REAL CUSTOMER STORIES ===',
      'CRITICAL: Only reference real customer experiences from these testimonials.',
      'DO NOT invent fictional characters, names, or specific scenarios.',
      'If telling a story, use anonymized versions of these real experiences.',
      ''
    ];

    stories.slice(0, 5).forEach((story, idx) => {
      sections.push(`[Testimonial ${idx + 1}]: "${story}"`);
    });

    sections.push('');
    sections.push('Use these real experiences to ground your content authentically.');
    sections.push('If no testimonial fits, use hypothetical framing like "A typical customer..." or "Many businesses find..."');
    sections.push('=== END CUSTOMER STORIES ===\n');

    console.log(`[V4 Orchestrator] Adding ${stories.length} customer stories for grounding`);
    return sections.join('\n');
  }
  ...
  async generate(
    ...
    try {
      ...
      // 5b. CRITICAL: Enhance with insights from Synapse 2.0 API data
      // If user provided selectedInsights, use those
      // If contentTopic provided, use semantic matching via embeddings
      // Otherwise, AUTO-INJECT top triggers/gaps from the Synapse data provider
      let insightsToUse = request.selectedInsights || [];
      if (insightsToUse.length === 0 && synapseDataProvider.hasData()) {
        // Try semantic matching if topic provided and embeddings ready
        if (request.contentTopic) {
          insightsToUse = await synapseDataProvider.getRelevantInsights(request.contentTopic, {
            maxTriggers: 3,
            maxGaps: 2,
          });
          console.log(`[V4 Orchestrator] Semantic selection: ${insightsToUse.length} insights for topic "${request.contentTopic.slice(0, 50)}..."`);
        } else {
          // Fall back to confidence-based selection
          insightsToUse = synapseDataProvider.getAutoSelectedInsights({
            maxTriggers: 3,
            maxGaps: 2,
          });
          console.log(`[V4 Orchestrator] Auto-injecting ${insightsToUse.length} Synapse 2.0 insights (no manual selection)`);
        }
      }
      if (insightsToUse.length > 0) {
        prompt += this.buildSelectedInsightsPrompt(insightsToUse);
      }

      // 5c. Add brand voice/tone guidance if available
      if (request.uvp.brandVoice) {
        prompt += this.buildBrandVoicePrompt(request.uvp.brandVoice);
      }

      // 5d. Add customer stories for grounding if available
      if (request.uvp.customerStories && request.uvp.customerStories.length > 0) {
        prompt += this.buildCustomerStoriesPrompt(request.uvp.customerStories);
      }
  ...
  private parseResponse(response: string, templateId: string): {
    ...
    try {
      ...
      // Handle ```json at start with optional whitespace/newlines
      cleaned = cleaned.replace(/^```(?:json)?\s*\n?/i, '');
      // Handle ``` at end with optional whitespace/newlines
      cleaned = cleaned.replace(/\n?\s*```\s*$/i, '');
      // Also handle inline backticks that might wrap JSON
      cleaned = cleaned.replace(/^`+|`+$/g, '');
      cleaned = cleaned.trim();

      // Find JSON in response - handle both objects and arrays
      // First try to find an array (multiple content items)
      ...
      if (jsonArrayMatch) {
        try {
          const jsonStr = jsonArrayMatch[0].replace(/^`+|`+$/g, '').trim();
          const arr = JSON.parse(jsonStr);
          // Use first item if it's an array
          parsed = Array.isArray(arr) && arr.length > 0 ? arr[0] : arr;
          console.log('[V4 Orchestrator] Parsed JSON array, using first item');
        } catch (e) {
          // Try object match instead
        }
      }

      if (!parsed && jsonObjectMatch) {
        // Clean any remaining backticks from the matched JSON
        const jsonStr = jsonObjectMatch[0].replace(/^`+|`+$/g, '').trim();
        parsed = JSON.parse(jsonStr);
      }

      if (parsed) {
        return {
          headline: parsed.headline || parsed.title || parsed.hook?.substring(0, 60),
          hook: parsed.hook || parsed.opening_line || parsed.openingLine || parsed.connection || '',
          body: this.buildBody(parsed),
          cta: parsed.cta || parsed.engagement_prompt || parsed.call_to_action || '',
          hashtags: parsed.hashtags || []
        };
      }
