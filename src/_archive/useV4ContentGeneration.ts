...
export interface UseV4ContentGenerationReturn {
  ...
  generateQuickPost: (platform?: 'linkedin' | 'instagram' | 'twitter' | 'facebook' | 'tiktok', selectedInsights?: SelectedInsight[]) => Promise<QuickPostResult>;
  ...
  const generateQuickPost = useCallback(async (
    ...
    try {
      ...
      if (selectedInsights?.length) {
        console.log(`[useV4ContentGeneration] Using ${selectedInsights.length} selected insights from tabs`);
      }
