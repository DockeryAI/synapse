...
class TriggerSynthesisService {
  ...
  // PHASE M: Synthesis mutex to prevent overlapping runs
  private synthesisInProgress = false;
  private currentSynthesisBrandId: string | null = null;
  ...
  /**
   * PHASE M: Check if synthesis is already running for a brand
   * Returns true if we should skip this synthesis request
   */
  isSynthesisInProgress(brandId?: string): boolean {
    if (!this.synthesisInProgress) return false;

    // If same brand, allow (it's likely a retry or continuation)
    if (brandId && this.currentSynthesisBrandId === brandId) {
      console.warn(`[TriggerSynthesis] Synthesis already in progress for brand ${brandId}, blocking duplicate`);
      return true;
    }

    // Different brand or no brandId - new synthesis should wait
    console.warn(`[TriggerSynthesis] Synthesis in progress, blocking new request`);
    return true;
  }

  /**
   * PHASE M: Reset synthesis mutex (call when synthesis completes or errors)
   */
  resetSynthesisMutex(): void {
    this.synthesisInProgress = false;
    this.currentSynthesisBrandId = null;
  }
  ...
  async generateTriggers(
    ...
    try {
      // Single Claude call with retry on 503
      const response = await this.callLLMWithRetry(prompt);
      // PHASE M: Pass samples for source-content validation
      const rawTriggers = this.parseV1Response(response, filteredSamples);
   ...
   * PHASE M: Added synthesis mutex to prevent overlapping runs
   */
  async runMultiPass(
    brandProfile: BrandProfile,
    rawSamples: RawDataSample[],
    ...
    // PHASE M: Check mutex - prevent overlapping synthesis runs
    if (this.synthesisInProgress) {
      console.warn(`[TriggerSynthesis] ⚠️ Synthesis already in progress, skipping duplicate request`);
      return {
        triggers: [],
        passSummary: [],
        totalDuration: 0,
    ...
    this.currentSynthesisBrandId = brandId || null;
    console.log(`[TriggerSynthesis] 🔒 Mutex acquired for brand: ${brandId || 'unknown'}`);
    ...
    try {
      ...
      console.log(`[TriggerSynthesis] 🔓 Mutex released - synthesis complete`);
...
Output ONLY the JSON array, no other text.`;
  ...
  /**
   * PHASE M: Call LLM with exponential backoff retry on 503/429 errors
   * Retries up to 3 times with delays: 1s, 2s, 4s
   */
  private async callLLMWithRetry(prompt: string, maxRetries: number = 3): Promise<string> {
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
      try {
        ...
        if (!isRetryable || attempt >= maxRetries) {
          console.error(`[TriggerSynthesis] LLM call failed after ${attempt} attempts:`, errorMessage);
          throw lastError;
        }

        // Exponential backoff: 1s, 2s, 4s
        const delayMs = Math.pow(2, attempt - 1) * 1000;
        console.warn(`[TriggerSynthesis] 503/429 error, retrying in ${delayMs}ms (attempt ${attempt}/${maxRetries})`);
  ...
  /**
   * Parse V1-style LLM response with source-content validation
   */
  private parseV1Response(response: string, samples?: RawDataSample[]): V1TriggerOutput[] {
    ...
    try {
      ...
      // Validate each trigger has required fields AND source-content validation
      return parsed.filter((t: any) => {
        ...
        // PHASE M: SOURCE-CONTENT VALIDATION
        // Verify the verbatim quote actually appears in one of the cited samples
        if (samples && samples.length > 0) {
          const validQuote = this.validateQuoteInSamples(t.verbatimQuote, t.sampleIds, samples);
          if (!validQuote) {
            console.warn('[TriggerSynthesis] ⚠️ REJECTED - quote not found in cited sources:', t.title);
            console.warn('[TriggerSynthesis] Quote:', t.verbatimQuote.substring(0, 60));
            return false;
          }
        }
  ...
  /**
   * PHASE M: Validate that a quote exists in the cited samples
   * Uses fuzzy matching to handle minor LLM paraphrasing (80% threshold)
   */
  private validateQuoteInSamples(quote: string, sampleIds: number[], samples: RawDataSample[]): boolean {
    if (!quote || quote.length < 10) return false;

    // Normalize quote for comparison
    const normalizedQuote = this.normalizeForComparison(quote);

    // Extract key phrases from the quote (words 4+ chars)
    const keyPhrases = normalizedQuote.split(/\s+/).filter(w => w.length >= 4);
    if (keyPhrases.length < 3) return true; // Skip validation for very short quotes

    // Check each cited sample
    for (const sampleId of sampleIds) {
      const sample = samples[sampleId - 1]; // 1-indexed
      if (!sample?.content) continue;

      const normalizedContent = this.normalizeForComparison(sample.content);

      // Method 1: Direct substring match (best case)
      if (normalizedContent.includes(normalizedQuote)) {
        return true;
      }

      // Method 2: Key phrase overlap (handles minor paraphrasing)
      // Require 60% of key phrases to appear in source
      const matchingPhrases = keyPhrases.filter(phrase =>
        normalizedContent.includes(phrase)
      );
      const overlapRatio = matchingPhrases.length / keyPhrases.length;
    ...
    // Quote not found in any cited source
    return false;
  }
