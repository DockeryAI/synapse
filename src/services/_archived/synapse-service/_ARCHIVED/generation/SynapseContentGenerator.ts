...
export class SynapseContentGenerator {
  ...
  async generate(
    ...
    for (const insight of insights) {
      try {
        ...
        console.error(`Error generating content for insight ${insight.id}:`, errorMessage);
  ...
  private selectFormats(
    ...
    return formats.slice(0, 3); // Max 3 formats per insight
  }
    ...
    try {
      switch (format) {
        ...
        case 'controversial-post':
          // Controversial posts have validation - may throw
      ...
      console.error(`Error generating ${format} for insight ${insight.id}:`, error);
      return null;
    }
  }
