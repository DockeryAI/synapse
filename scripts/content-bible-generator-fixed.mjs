...
/**
 * Content Writing Bible Generator - FIXED VERSION
 * Queries top 10 LLMs via OpenRouter for comprehensive content writing best practices
 * SAVES individual responses AND synthesizes all responses into one master document
 */
...
// Top 10 LLMs for comprehensive research (FIXED model identifiers)
const TOP_LLMS = [
  { id: 'anthropic/claude-3.5-sonnet', name: 'Claude 3.5 Sonnet' },
  { id: 'anthropic/claude-3-opus', name: 'Claude 3 Opus' },
  { id: 'openai/gpt-4-turbo', name: 'GPT-4 Turbo' },
  { id: 'openai/gpt-4o', name: 'GPT-4o' },
  { id: 'google/gemini-pro-1.5', name: 'Gemini Pro 1.5' }, // FIXED correct ID
  { id: 'meta-llama/llama-3.1-405b-instruct', name: 'Llama 3.1 405B' },
  { id: 'mistralai/mistral-large', name: 'Mistral Large' },
  { id: 'anthropic/claude-3-haiku', name: 'Claude 3 Haiku' }, // Replaced broken Cohere
  { id: 'deepseek/deepseek-chat', name: 'DeepSeek Chat' },
  { id: 'qwen/qwen-2.5-72b-instruct', name: 'Qwen 2.5 72B' } // Replaced broken Perplexity
];

// Fallback models if any fail
const FALLBACK_MODELS = [
  { id: 'openai/gpt-4o-mini', name: 'GPT-4o Mini' },
  { id: 'google/gemini-flash-1.5', name: 'Gemini Flash 1.5' }, // FIXED
...
async function saveIndividualResponse(response, index) {
  ...
  const filename = `llm-response-${index}-${response.model.replace(/[\/\s]/g, '-')}-${timestamp}.md`;
...
${response.content}`;
  ...
  console.log(`\n💾 Saved response from ${response.model} to content-writing-guide/LLM-raw-data/${filename}`);
  return filepath;
}
...
// Synthesize all responses into master document (FIXED)
async function synthesizeResponses(responses) {
  console.log('\n\n📚 Synthesizing responses from all LLMs...\n');

  // FIXED: More explicit prompt that demands complete output
  const synthesisPrompt = `You are tasked with synthesizing ${responses.length} comprehensive responses about content writing best practices into one COMPLETE master document.

CRITICAL INSTRUCTIONS:
1. This is a SINGLE-SHOT API CALL - you must provide the ENTIRE document in THIS response
2. Do NOT break this into multiple messages or ask if you should continue
3. Generate the COMPLETE guide from start to finish
4. Include ALL sections with full detail, not just an outline
5. Synthesize and expand on ALL content from the responses below
...
IMPORTANT: Generate the COMPLETE document NOW. Do not stop partway or ask to continue. This must be a full, comprehensive guide that includes:
- Complete table of contents
- Full executive summary
- All 12 main sections with complete content (not just headers)
- All subsections fully written out
- All examples, templates, and formulas
- Complete quick win checklist
- Full conclusion

The final document should be AT LEAST 10,000 words and be the ultimate, comprehensive guide to content writing that drives business results. It should be structured, detailed, and immediately actionable.

Format the output in clean Markdown with proper headers, lists, and emphasis where appropriate.

BEGIN THE COMPLETE DOCUMENT NOW:`;
...
// Main execution
async function main() {
  console.log('🚀 Content Writing Bible Generator (FIXED VERSION) Starting...\n');
  ...
  console.log('✨ NEW: Individual responses will be saved to content-writing-guide/LLM-raw-data/\n');
...
- **Individual Responses Saved:** ${savedFiles.length} files in llm-responses/ folder
...
`;
  ...
  // Save to file
  const timestamp = new Date().toISOString().replace(/[:.]/g, '-').slice(0, -5);
  const filename = `content-writing-bible-COMPLETE-${timestamp}.md`;
  const filepath = path.join(__dirname, '..', 'content-writing-guide', filename);
  ...
  console.log('\n📄 Files saved:');
  console.log(`  • Main document: ${filepath}`);
  ...
  console.log('  insights from multiple leading AI models. Individual responses have been');
  console.log('  preserved for reference, and all content has been synthesized into a');
  console.log('  complete master document.');
  console.log('\n💡 Next steps: Open the main file to access your comprehensive guide.');
  console.log('\n' + '='.repeat(80) + '\n');
}
