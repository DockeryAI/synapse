-- FORCE FIX: location_detection_cache table 400 errors
-- Date: 2025-11-22
...
CREATE POLICY "cache_select_v6"
ON public.location_detection_cache
FOR SELECT
TO public
USING (true);

CREATE POLICY "cache_insert_v6"
ON public.location_detection_cache
FOR INSERT
TO public
WITH CHECK (true);

CREATE POLICY "cache_update_v6"
ON public.location_detection_cache
FOR UPDATE
TO public
USING (true)
...
BEGIN
  ...
  FOR pol IN
    ...
    WHERE tablename = 'location_detection_cache'
  LOOP
    RAISE NOTICE '  - % (%) for %', pol.policyname, pol.cmd, pol.role_list;
  END LOOP;
