...
-- Add some demo content for testing (optional, can be removed in production)
-- This will only work if there's a valid brand in the database
DO $$
DECLARE
  sample_brand_id UUID;
BEGIN
  -- Get a sample brand ID if one exists
  SELECT id INTO sample_brand_id FROM public.brands LIMIT 1;

  IF sample_brand_id IS NOT NULL THEN
    -- Insert sample content items
    INSERT INTO public.content_calendar_items (brand_id, title, content, platform, status, scheduled_for)
    VALUES
      (sample_brand_id, 'Welcome Post', 'Welcome to our new social media presence!', 'instagram', 'scheduled', NOW() + INTERVAL '1 day'),
      (sample_brand_id, 'Product Spotlight', 'Check out our featured product of the week', 'facebook', 'draft', NOW() + INTERVAL '3 days'),
      (sample_brand_id, 'Industry Insights', 'Top 5 trends in our industry this year', 'linkedin', 'scheduled', NOW() + INTERVAL '5 days');
  END IF;
END $$;