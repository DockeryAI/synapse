/**
 * Save Buyer Personas to Database
 *
 * This script saves buyer personas to the buyer_personas table
 * using the existing OnboardingV5DataService
 */

import { OnboardingV5DataService } from './src/services/supabase/onboarding-v5-data.service.js';

const brandId = '001e28bd-afa4-43e1-a262-7a459330cd01'; // OpenDialog brand ID

// Buyer personas based on the existing OpenDialog UVP
const buyerPersonas = [
  {
...
async function saveBuyerPersonas() {
  ...
  try {
    await OnboardingV5DataService.saveBuyerPersonas(brandId, buyerPersonas);
    console.log('✅ Successfully saved buyer personas to database');

    // Load them back to verify
    console.log('\nVerifying saved personas...');
    const savedPersonas = await OnboardingV5DataService.loadBuyerPersonas(brandId);
    console.log(`✅ Found ${savedPersonas.length} personas in database:`);
    savedPersonas.forEach(persona => {
      console.log(`  - ${persona.persona_name} (${persona.role?.title})`);
    });
